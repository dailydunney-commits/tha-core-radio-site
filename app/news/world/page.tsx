...
export default function NewsCategoryPage() {
  ...
  async function loadNews() {
    setLoading(true);
    try {
      const res = await fetch("/api/news?category=world", { cache: "no-store" });
  ...
  const tickerText =
    items.length > 0
      ...
      : "Loading live World News headlines...";
    ...
    <main className="min-h-screen bg-black px-6 py-10 text-white">
      ...
      <section className="mx-auto max-w-6xl">
        ...
        <div className="mt-8 rounded-[2rem] border-2 border-red-600 bg-gradient-to-br from-yellow-200 via-amber-300 to-yellow-500 p-8 text-black">
          <p className="text-sm font-black tracking-[0.4em]">GLOBAL HEADLINES</p>
          <h1 className="mt-3 text-5xl font-black md:text-7xl">World News</h1>
          <p className="mt-4 max-w-3xl text-lg font-bold">Live world headlines and breaking stories.</p>
