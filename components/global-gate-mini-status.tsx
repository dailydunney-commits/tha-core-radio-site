...
export default function GlobalGateMiniStatus() {
  ...
  const [testResult, setTestResult] = useState("No gate test run yet.");
  const [testing, setTesting] = useState(false);

  function publishGateResult(message: string) {
    setTestResult(message);

    window.dispatchEvent(
      new CustomEvent("tha-core-global-gate-result", {
        detail: {
          message,
        },
      })
  ...
  async function runGateTest(kind: "safe" | "dirty") {
    setTesting(true);
    setTestResult(kind === "safe" ? "Testing approved clean audio..." : "Testing dirty audio hold...");
    ...
    try {
      const body =
        kind === "safe"
          ? {
              ...
              mode: "owner_panel_test",
              track: {
                id: "owner-safe-test",
                title: "Owner Safe Test",
                artist: "Tha Core",
          ...
          : {
              ...
              track: {
                id: "owner-dirty-test",
                title: "Owner Dirty Explicit Test",
                artist: "Tha Core",
                reason: "dirty explicit uncensored test",
                audioUrl: "/audio/smartdj/test-bleeped-clean.mp3",
              },
            };

      const res = await fetch("/api/radio/global-audio-gate", {
        method: "POST",
        headers: {
      ...
      const message = data?.allowBroadcast
        ? `APPROVED ✅ ${data?.decision || "ALLOW"}`
        : `HELD ⛔ ${data?.decision || "BLOCKED"}`;

      publishGateResult(message);
    } catch {
      publishGateResult("Gate test failed.");
    } finally {
      setTesting(false);
      loadStatus();
    }
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button type="button" onClick={() => runGateTest("safe")} disabled={testing} style={buttonStyle}>
            TEST APPROVED
          </button>
          <button type="button" onClick={() => runGateTest("dirty")} disabled={testing} style={buttonStyle}>
            TEST HELD
          </button>
          <button type="button" onClick={loadStatus} disabled={testing} style={buttonStyle}>
            REFRESH GATE
          </button>
        </div>
      </div>
      ...
      <p style={{ margin: "10px 0 0", color: "#00d9ff", fontSize: 12, fontWeight: 900 }}>
        Last Test: {testResult}
      </p>

      <p style={{ margin: "6px 0 0", color: "#ffeb66", fontSize: 12, fontWeight: 800 }}>
